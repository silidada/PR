...
class Model_depth_wise(nn.Module):
    def __init__(self):
        super(Model_depth_wise, self).__init__()
        # input (128*128*1) -> (128,128,32)
        self.conv1 = nn.Conv2d(1, 32, (3, 3), padding=1, stride=(1,1))
        self.batchNorm1 = nn.BatchNorm2d(32)
        # input (64*64*32) -> (64,64,64)
        ...
        # input (64*64*64) -> (64*64*64)
        self.block11 = ResidualBlock1(64)
        # input (64*64*64) -> (32*32*128)
        self.block12 = ResidualBlock2(64)
        # input (32*32*128) -> (32*32*128)
        self.block13 = ResidualBlock1(128)
        # input (32*32*128) -> (16*16*256)
        self.block14 = ResidualBlock2(128)
        # input (16*16*256) -> (16*16*256)
        self.block15 = ResidualBlock1(256)
        # input (16*16*256) -> (8*8*512)
        self.block16 = ResidualBlock2(256)
        # input (8*8*512) -> (8*8*256)
        self.conv7 = nn.Conv2d(512, 256, (1, 1), padding=0, stride=(1, 1))
        self.batchNorm7 = nn.BatchNorm2d(256)
        # input (8*8*256) -> (8*8*256)
        self.block17 = ResidualBlock1(256)
        # input (8*8*256) -> (4*4*512)
        self.block18 = ResidualBlock2(256)
        # input (4*4*512) -> (4*4*512)
        self.block19 = ResidualBlock1(512)
        # input (4*4*512) -> (2*2*1024)
        self.block20 = ResidualBlock2(512)

        # input (64*64*64) -> (64*64*64)
        self.block21 = ResidualBlock1(64)
        # input (64*64*64) -> (32*32*128)
        self.block22 = ResidualBlock2(64)
        # input (32*32*128) -> (32*32*128)
        self.block23 = ResidualBlock1(128)
        # input (32*32*128) -> (16*16*256)
        self.block24 = ResidualBlock2(128)
        # input (16*16*256) -> (16*16*256)
        self.block25 = ResidualBlock1(256)
        # input (16*16*256) -> (8*8*512)
        self.block26 = ResidualBlock2(256)
        # input (8*8*512) -> (8*8*256)
        self.conv8 = nn.Conv2d(512, 256, (1, 1), padding=0, stride=(1, 1))
        self.batchNorm8 = nn.BatchNorm2d(256)
        # input (8*8*256) -> (8*8*256)
        self.block27 = ResidualBlock1(256)
        # input (8*8*256) -> (4*4*512)
        self.block28 = ResidualBlock2(256)
        # input (4*4*512) -> (4*4*512)
        self.block29 = ResidualBlock1(512)
        # input (4*4*512) -> (2*2*1024)
        self.block30 = ResidualBlock2(512)

        self.downSample = nn.Conv2d(3072, 2048, (1, 1), padding=0, stride=(1, 1))
        self.downSample1 = nn.Conv2d(2048, 1024, (1, 1), padding=0, stride=(1, 1))
    ...
    def forward(self, x):
        ...
        out1 = F.relu(self.batchNorm3(self.conv3(out1)))
        out1 = self.block7(out1)
        out1 = self.block8(out1)
        out1 = self.block9(out1)
        out1 = self.block10(out1)

        out2 = self.block11(out)
        out2 = self.block12(out2)
        out2 = self.block13(out2)
        out2 = self.block14(out2)
        out2 = self.block15(out2)
        out2 = self.block16(out2)
        out2 = F.relu(self.batchNorm7(self.conv7(out2)))
        out2 = self.block17(out2)
        out2 = self.block18(out2)
        out2 = self.block19(out2)
        out2 = self.block20(out2)

        out3 = self.block21(out)
        out3 = self.block22(out3)
        out3 = self.block23(out3)
        out3 = self.block24(out3)
        out3 = self.block25(out3)
        out3 = self.block26(out3)
        out3 = F.relu(self.batchNorm8(self.conv8(out3)))
        out3 = self.block27(out3)
        out3 = self.block28(out3)
        out3 = self.block29(out3)
        out3 = self.block30(out3)
...
class Model_depth_wise_fc(nn.Module):
    def __init__(self):
        super(Model_depth_wise_fc, self).__init__()
        # input (128*128*1) -> (128,128,32)
        self.conv1 = nn.Conv2d(1, 32, (3, 3), padding=1, stride=(1,1))
        self.batchNorm1 = nn.BatchNorm2d(32)
        # input (64*64*32) -> (64,64,64)
...
if __name__ == '__main__':
    test_tensor = torch.randn((254,1,128,128))
